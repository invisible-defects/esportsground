...
function updateLeftMenu() {
  var scrollY = scrollGetY(), dy = scrollY - (__leftMenu.lastScroll || 0);
  ...
  var menu = lm.bar, pageBody = lm.pb;
  if (browser.mobile || !menu || !pageBody) return;

  var wh = window.lastWindowHeight || 0, st = Math.min(scrollY, bodyNode.clientHeight - wh),
      pl = lm.pl, headH = getPageHeaderHeight(),
      headCalcH = Math.min(Math.max(0, headH - st), headH),
      isFixed = __leftMenu.lmFixed,
      menuH = getSize(menu)[1], menuPos = isFixed ? getXY(menu)[1] : __leftMenu.lmMarginTop,
      pageH = getSize(pageBody)[1], pagePos = pageBody.offsetTop, tooBig = menuH >= pageH,
      lastSt = __leftMenu.menuLastSt || 0, lastStyles = __leftMenu.menuLastStyles || {}, styles, delta = 1,
...
function showAudioClaimWarning(audio, claim, onReplace) {
  ...
  if (onReplace && original) {
    ...
    params.push(getLang('audio_replace_with_original'), function() {
      ...
      onReplace(function () { dialog.hide(); });
    });
...
function moneyTransferBox(txId, hash, ev, btn, decline, request, force) {
  ...
  var params;
  if (request) {
    params = {act: 'money_transfer_box', request_id: txId, request: request, hash: hash};
  } else {
    params = {act: 'accept_money_transfer_box', tx_id: txId, hash: hash};
  }
 ...
 * @param opts
       ...
       center: bool - заставляет уголок выводиться в центру стороны, на которой он находится
...
function mobileOnlineTip(el, opts) {
  ...
  return showTooltip(el, {
    url: 'al_login.php',
    params: {act: 'mobile_tt', mid: opts.mid, was: opts.was, vk_mobile: opts.vk_mobile},
    slide: 15,
    ajxdt: 200,
    showdt: 200,
    hidedt: 200,
...
function pageVerifiedTip(el, opts) {
  return showTooltip(el, {
    url: '/al_page.php',
    params: {act: 'verified_tt', type: opts.type, oid: opts.oid},
    slide: 15,
    ajxdt: 200,
    showdt: 200,
    hidedt: 200,
